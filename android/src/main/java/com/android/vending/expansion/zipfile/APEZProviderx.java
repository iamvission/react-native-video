...
//To implement APEZProviderx in your application, you'll want to change
//the AUTHORITY to match what you define in the manifest.

import com.android.vending.expansion.zipfile.ZipResourceFilex.ZipEntryRO;
 ...
 *
 */
public abstract class APEZProviderx extends ContentProvider {	

	private ZipResourceFilex mAPKExtensionFile;
	    ...
	    if ( !mInit ) {
            ...
            try {
                if ( null == resourceFiles ) {
                    mAPKExtensionFile = APKExpansionSupportx.getAPKExpansionZipFile(ctx, mainFileVersion, patchFileVersion);
                } else {
                    mAPKExtensionFile = APKExpansionSupportx.getResourceZipFile(resourceFiles);
                }
                mInit = true;
                return true;
            } catch (IOException e) {
